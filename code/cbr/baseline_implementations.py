...
class PureAIBaseline:
    ...
    def __init__(self, llm_client):
        if llm_client is None:
            raise ValueError("PureAIBaseline requires real LLM client!")
    ...
    def teach(self, query: Dict) -> Dict:
        """Generate REAL LLM response without case grounding"""
        
        # Call REAL LLM
        response = self.llm_client.generate_teaching_response(
            misconception=query.get('misconception', 'unknown'),
            context=query.get('topic', 'mathematics'),
            strategy=query.get('persona_type', 'socratic'),
            temperature=0.7
        )
        
        # Score based on response quality
        # (In production, would test with real students)
        base_score = self.global_stats.get('mean_success', 0.65)
        
        # Simple heuristic: longer, more detailed = better
        detail_factor = min(len(response) / 300.0, 1.5)
        adjusted_score = base_score * (0.85 + 0.15 * detail_factor)
        
        return {
            'score': adjusted_score,
            'strategy': 'pure_ai',
            'response': response  # Include actual response!
        }
...
class HybridMnemonicSystem:
    ...
    def __init__(self, mnemonic_engine, llm_client):
        if llm_client is None:
            raise ValueError("HybridMnemonicSystem requires real LLM client!")
    ...
    def retrieve_and_generate(self, query: Dict) -> Dict:
        ...
        # Generate LLM response WITH case grounding
        retrieved_case_info = [
            {
                'misconception': case.misconception,
                'success_rate': case.outcome
            }
            for _, case in top_cases[:3]  # Top 3 cases
        ]
        ...
        response = self.llm_client.generate_teaching_response(
            ...
            context=query.get('topic', 'mathematics'),
            strategy=query.get('persona_type', 'socratic'),
            retrieved_cases=retrieved_case_info,  # Ground in cases!
            temperature=0.7
        )
