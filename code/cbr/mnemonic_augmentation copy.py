...
class MnemonicAugmentation:
    ...
    def _identify_prerequisites(self, case: Case) -> List[str]:
        ...
        # Simplified - would be extracted from intervention details in full implementation
    ...
    def _identify_common_errors(self, case: Case) -> List[str]:
        ...
        # Simplified - would analyze actual student errors in full implementation
        return ['procedural_error', 'conceptual_confusion']
    
    # ============ ENHANCED RETRIEVAL ============
    def enhanced_similarity(self, query: Case, candidate: Case) -> float:
        """
        Compute enhanced similarity using mnemonic techniques.
        
        This replaces standard k-NN similarity with mnemonic-augmented version.
        """
        # CRITICAL FIX: Normalize features FIRST, then weight by correlation
        # This prevents large-magnitude features from dominating similarity
        
        # Feature ranges from EEDI dataset:
        # feature[0]: QuizId (0)
        # feature[1]: QuestionId (0-879)
        # feature[2]: misconception_count (1-4)
        # feature[3]: construct_length (21-165)
        feature_ranges = np.array([1.0, 900.0, 4.0, 170.0])  # Max values for normalization
        
        # Normalize both query and candidate to [0, 1]
        query_normalized = query.features / feature_ranges
        candidate_normalized = candidate.features / feature_ranges
        
        # Get correlation weights for each feature
        feature_weights = np.zeros(len(query_normalized))
        for i in range(len(query_normalized)):
            weight_key = f"feature_{i}"
            if weight_key in self.retrieval_cue_weights:
                feature_weights[i] = self.retrieval_cue_weights[weight_key]
        
        # Normalize weights to sum to 1
        weight_sum = np.sum(feature_weights)
        if weight_sum > 0:
            feature_weights = feature_weights / weight_sum
        else:
            # Fallback to uniform weights if no weights available
            feature_weights = np.ones(len(query_normalized)) / len(query_normalized)
        
        # Apply weights to NORMALIZED features
        query_weighted = query_normalized * feature_weights
        candidate_weighted = candidate_normalized * feature_weights
        
        # Base similarity (cosine similarity of NORMALIZED + WEIGHTED features)
        # IMPORTANT: Cosine similarity ranges from -1 to +1
        # We clip to [0, 1] since negative similarity = dissimilar = not useful
        base_sim_raw = np.dot(query_weighted, candidate_weighted) / (
            np.linalg.norm(query_weighted) * np.linalg.norm(candidate_weighted) + 1e-10
        )
        base_sim = max(0.0, base_sim_raw)  # Clip negative similarities to 0
        ...
        # 4. Elaboration coherence - check if elaborations align
        ...
        # CRITICAL FIX: DO NOT multiply by utility_score (alpha)!
        # All cases have similar utility_score (~0.5), which flattens similarities
        # and destroys discrimination power. We want base_sim differences preserved.
        # The correlation-weighted similarity already captures effectiveness.
        
        enhanced_sim = base_sim * beta  # Only apply mnemonic enhancements, not utility_score
        
        # Ensure final similarity is in reasonable range [0, ~2]
        # (can be > 1 due to enhancements, but shouldn't be huge)
...
def create_case_from_eedi(row: pd.Series, case_id: str) -> Case:
    ...
    # Extract features with robust type conversion
    # CRITICAL FIX: Only use 4 features to match system expectations
    features = np.array([
        safe_numeric(row.get('QuizId', 0)) % 1000,  # Normalize to reasonable range
        safe_numeric(row.get('QuestionId', 0)) % 1000,
        safe_numeric(row.get('misconception_count', 0)),
        len(str(row.get('ConstructName', ''))),
        # REMOVED: 1.0 if pd.notna(row.get('MisconceptionAName')) else 0.0,  # 5th feature removed
    ...
    # Outcome: Generate SIMPLE, STRONG relationship with features
    ...
    # REMOVED: has_misconception = features[4]  # No longer have 5th feature
    
    # SIMPLE linear combination - easy to learn!
    # Make misconception count the dominant factor
    feature_effect = (
        0.15 * norm_question_id +      # Minor effect
        0.65 * norm_misconception +    # MAJOR effect (most predictive) - increased from 0.60
        0.20 * norm_construct_len      # Moderate effect
        # REMOVED: 0.05 * has_misconception  # No 5th feature
    )
    # Total: 1.00
...
def load_cases_from_eedi(
    ...
    if sample_from_full:
        ...
        # Strategy: Use skiprows with random selection
        logger.info("Sampling from FULL dataset (this may take a while)...")
        
        # First, count total rows (read just first column)
        ...
        df = pd.read_csv(
            filepath,
            skiprows=lambda i: i > 0 and (i-1) in skip_rows  # Skip header (i=0) then check rows
        )
